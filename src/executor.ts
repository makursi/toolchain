...
async function writePlanFile(
  root: string,
  file: InitPlan["files"][number],
): Promise<"written" | "skipped"> {
  const full = join(root, file.path);
  if (file.action === "keep-old") return "skipped";
...
async function applyPackageJson(root: string, plan: InitPlan): Promise<void> {
  ...
  if (packageJson.simpleGitHooks) pkg["simple-git-hooks"] = packageJson.simpleGitHooks;
  if (packageJson.lintStaged) pkg["lint-staged"] = packageJson.lintStaged;
