...
function decideAction(existing: string | undefined, planned: string, flags: InitFlags): { action: FileAction; unchanged: boolean } {
  if (existing === undefined)
    return { action: 'create', unchanged: false }
  if (existing === planned)
    return { action: 'keep-old', unchanged: true }
...
function planPackageJson(ctx: ProjectContext, hasPackageJson: boolean): PlanPackageJson {
  ...
  for (const [name, command] of Object.entries(toolchain)) {
    if (!ctx.existingScripts.has(name))
      scripts[name] = command
  }
  for (const [name, range] of Object.entries(deps)) {
    if (!ctx.existingDevDependencies.has(name))
...
function planCollisions(ctx: ProjectContext): Array<{ name: string; existing: string; planned: string }> {
...
function planSteps(ctx: ProjectContext, pkg: PlanPackageJson): InitPlan['steps'] {
  ...
  if (ctx.framework === 'next') {
    steps.push({
      kind: 'notice',
      message: 'Next.js detected: tsconfig.json is managed by Next.js itself, so the toolchain did not touch it.',
    })
  }
  if (!ctx.hasPackageJson && Object.keys(pkg.scripts).length === 0) {
    steps.push({ kind: 'notice', message: 'No package.json found — config files were written, but scripts/deps need a package.json to merge into.' })
  }
  return steps
}
...
export function planIsEmpty(plan: InitPlan): boolean {
  ...
  const edits = Object.keys(plan.packageJson.scripts).length + Object.keys(plan.packageJson.devDependencies).length
