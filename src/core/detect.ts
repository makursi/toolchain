...
export function detectMonorepo(root: string): boolean {
  ...
  const pkg = readJsonIfPresent(root, "package.json");
  if (pkg && (pkg.workspaces !== undefined || pkg.pnpm !== undefined)) return true;
  return false;
}
...
function existingPkgFields(root: string) {
  ...
  if (pkg) {
    ...
    if (rawScripts) {
      for (const [k, v] of Object.entries(rawScripts)) scripts.set(k, v);
    }
    if (rawDev) {
      for (const [k, v] of Object.entries(rawDev)) devDeps.set(k, v);
    }
  }
