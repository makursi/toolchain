...
cli.command("lint", "Run oxlint in the current project (read-only, no install)").action(() => {
  process.exitCode = runTool(process.cwd(), "lint");
});

cli.command("typecheck", "Run tsc --noEmit in the current project (read-only)").action(() => {
  process.exitCode = runTool(process.cwd(), "typecheck");
});

cli.command("fmt", "Run oxfmt in the current project (read-only, no install)").action(() => {
