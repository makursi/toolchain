...
export async function runInit(root: string, flags: InitFlags): Promise<number> {
  ...
  if (flags.dryRun) {
    ...
    if (s.length)
      log.message(`  ${pc.dim('·')} package.json scripts: ${s.join(', ')}`)
    if (d.length)
      log.message(`  ${pc.dim('·')} package.json devDependencies: ${d.join(', ')}`)
    for (const step of plan.steps)
      log.message(`  ${pc.dim('$')} ${step.message}`)
  ...
  if (sKeys.length)
    log.message(`  ${pc.dim('·')} package.json scripts: ${sKeys.join(', ')}`)
  if (dKeys.length)
  ...
  try {
    ...
    if (result.written.length)
      log.message(`  ${pc.dim('·')} wrote ${result.written.join(', ')}`)
    if (result.skipped.length)
      log.message(`  ${pc.dim('·')} kept existing: ${result.skipped.join(', ')}`)
    if (result.installed)
      log.message(`  ${pc.dim('·')} installed devDependencies`)
  }
  catch (err) {
    s.stop('Failed.')
  ...
  for (const step of plan.steps) {
    if (step.kind === 'install')
      log.message(pc.dim(`  next: ${step.message}`))
    else
      log.message(pc.dim(`  note: ${step.message}`))
  }
